...
def update_collection_visibility(self, context):
    for x in context.scene.render_schedule[context.scene.render_schedule_index].collections:
        if (self == x):
...
class VisibleCollection(bpy.types.PropertyGroup):
    ...
    visible:    bpy.props.BoolProperty(name='Visible', default=True, update=update_collection_visibility)


formats_list = (('PNG', 'PNG', '.png'), ('JPEG', 'JPEG', '.jpg'), ('OPEN_EXR', 'OpenEXR', '.exr'), ('OPEN_EXR_MULTILAYER', 'OpenEXR Multilayer', '.exr'))
codecs_list = (('ZIP', 'ZIP', 'ZIP'), ('DWAA', 'DWAA', 'DWAA'))

class RenderScheduleEntry(bpy.types.PropertyGroup):        
    name:                   bpy.props.StringProperty(name="Name")
    filepath:               bpy.props.StringProperty(name="Filepath", subtype = 'DIR_PATH', description='Path to output folder')
    compositor:             bpy.props.BoolProperty(name='Use Compositor', default=True)
    samples:                bpy.props.IntProperty(name='Samples', description='Sample count to be used in this render')
    ...
    color_depth:            bpy.props.EnumProperty(
                                name = "Color Depth",
                                items = (('8', '8', '(8 bit Depth)'), ('16', '16', '(16 bit Depth)'), ('32', '32', '(32 bit Depth)'))    
                            )
    float_depth:            bpy.props.EnumProperty(
                                name = "Color Depth",
                                items = (('16', 'Half', '(16 bit Depth)'), ('32', 'Full', '(32 bit Depth)'))    
                            )
    compression_quality:    bpy.props.IntProperty(name="Compression", default=15, subtype='PERCENTAGE', min=0, max = 100)
    ...
    camera:     bpy.props.PointerProperty(name="Camera", type=bpy.types.Object, poll=only_cameras_poll, description='Camera to be used for this render')
    
    world:     bpy.props.PointerProperty(name="World", type=bpy.types.World, description='World to be used for this render')
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            if(item.collection is None):
                return
            layout.label(text=item.collection.name, icon=['OUTLINER_COLLECTION', 'COLLECTION_' + item.collection.color_tag][item.collection.color_tag != 'NONE'])
            layout.prop(item, "visible", text='', icon=['RESTRICT_RENDER_ON', 'RESTRICT_RENDER_OFF'][item.visible], emboss=False)
...
class VIEW3D_UL_schedule(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            row.prop(item, "show_details", text="", toggle=1, icon=['TRIA_RIGHT', 'TRIA_DOWN'][item.show_details], emboss = False)
            row = row.split(factor=.9)
            row.prop(item, "name", text="", emboss=False)
            row = row.split()
            row.prop(item, "active", icon_only=True, icon=['RESTRICT_RENDER_ON', 'RESTRICT_RENDER_OFF'][item.active], emboss=False)
            ...
            if item.show_details:
                ...
                row = box.row()
                row.label(text=f'Visible Collections:\t\t{len([x for x in item.collections if x.visible])}')
                row.operator("scene.update_collections", text='', icon='FILE_REFRESH')
                row = box.row()
                row.template_list("VIEW3D_UL_collectionlsit", "", item, "collections", item, "collections_idx")
                row = box.row()
...
class VIEW3D_PT_render_scheduler(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = self.layout.row()
        row.template_list("VIEW3D_UL_schedule", "", context.scene, "render_schedule", context.scene, "render_schedule_index")
    ...
    def modal(self, context, event):
        if event.type == 'TIMER':
            if self.stop:
                ...
                if (item.name == '' or item.filepath == '' or item.camera is None or item.world is None):
...
#               self.properties.bl_rna.properties['preset_enum'].enum_items
                ...
                if (item.format == 'PNG'):
                    bpy.context.scene.render.image_settings.compression = item.compression_quality
                elif(item.format == 'JPEG'):
                    bpy.context.scene.render.image_settings.quality = item.compression_quality
                else: 
                    bpy.context.scene.render.image_settings.exr_codec = (item.codec)


                filepath = path.join(item.filepath, item.name + [x for x in formats_list if x[0]==item.format][0][2])
...
def register():
    ...
    bpy.types.Scene.render_schedule = bpy.props.CollectionProperty(type=RenderScheduleEntry)
    bpy.types.Scene.render_schedule_index = bpy.props.IntProperty(name="Index for render_schedule", default=0, update=item_selected)
